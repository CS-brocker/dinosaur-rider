...
1.    나를 i 프로세스라고 가정할 때, (A) lock이 0이면 첫 while에서 compare_and_swap이 성공해서 key에 이전 lock 값(0) 이 들어가고(key != 1), while을 빠져나가 임계구역 실행. 또는 (B) 다른 프로세스가 waiting[i] = false로 바꿔주면 첫 while 조건이 깨져(waiting[i] == false) while을 빠져나가 임계구역 실행.
2.    다른 프로세스가 진입하려고 할 때 lock이 이미 1이면 compare_and_swap이 계속 실패해서 key가 1로 남아 첫 while 조건을 못 깨고 임계구역에 진입하지 못함 ⇒ 상호 배제
3.    i 프로세스는 원형으로 돌면서 기다리는 프로세스를 찾고( waiting[j] == true 인 j ), j가 있으면 waiting[j] = false로 바꿔주며 임계구역 진입을 허가함(턴 넘김). 없으면 lock = 0으로 해제 ⇒ 한정된 대기
